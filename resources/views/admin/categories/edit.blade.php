...
<div class="container">
  ...
  <div class="box box-info">
    <div class="box-header with-border">
      <h3 class="box-title">{{__('Update Category')}}</h3>
    </div>
    <!-- /.box-header -->
    <!-- form start -->
    <form class="form-horizontal" method="POST" action="{{ route('admin.categories.update', $category->id) }}" enctype="multipart/form-data">
      ...
      <div class="box-body">
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <input style="width: 545px" type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="Name" value="{{ old('name') ?? $category->name}}" autocomplete="name" autofocus>
          </div>
        @error('name')
        ...
        <div class="form-group row">
          <label for="parent_id" class="col-sm-2 control-label">{{ __('Parent') }}</label>
           <div class="col-md-6">
              <select id="parent_id" class="form-control @error('parent_id') is-invalid @enderror" name="parent_id" value="{{ old('parent_id' ?? $category->parent_id) }}">
