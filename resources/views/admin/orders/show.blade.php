...
<a class="btn btn-dark" href="{{ route('admin.orders.edit', $order->id)}}">Edit</a>
...
<div>Show order</div>
<h3>Name: {{$order->user_id}}</h3>
<h3>Name: {{$order->total_price}}</h3>
<h3>Name: {{$order->description}}</h3>
<h3>Create by: {{$order->user  ? $order->user->name : ''}}</h3>
<h3>Create by: {{$order->user  ? $order->user->id : ''}}</h3>
